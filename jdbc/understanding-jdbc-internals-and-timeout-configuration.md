...
　![图2 Timeout Class](../sources/images/jdbc/timeout_class.png)  
　　　图2 Timeout Class.
...
|     JDBC Driver    | connectTimeout<br>配置项 |socketTimeout<br>配置项|  url格式  | 示例 
|----------------------|:------------:|:---------:|:------|------|   
|MySQL Driver   |connectTimeout<br>（默认值：0，单位：ms）|socketTimeout<br>（默认值：0，<br>单位：ms）|jdbc:mysql://[host:port],[host:port]…/[database][?propertyName1][=propertyValue1][&propertyName2][=propertyValue2]…|jdbc:mysql://xxx.xx.xxx.xxx:3306/database?connectTimeout=60000&socketTimeout=60000 
|MS-SQL DriverjTDS Driver|loginTimeout<br>（默认值：0，单位：s）|socketTimeout<br>（默认值：0，<br>单位：s）|jdbc:jtds:\<server_type\>://\<server\>[:\<port\>][/\<database\>][;\<property\>=\<value\>[;...]] |jdbc:jtds:sqlserver://server:port/database;loginTimeout=60;socketTimeout=60
